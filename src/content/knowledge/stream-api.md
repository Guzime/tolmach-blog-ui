...
В целом хочется начать про то, что Stream API это реализация функционального программирования в Java. Не забываем задачи функционального программирования, то это *как сделать программы проще для понимания, композиции, тестирования и параллельного выполнения, уменьшая количество изменяемого состояния и побочных эффектов.*
...
Его можно запустить используя функцию **`parallel()`** либо **`parallelStream()`**. Под капотом используется *ForkJoinPool*, здесь важно понимать что для него подходять задачи расчитанные
 для `CPU`, т.е. связанные с большими расчетами, которые можно поделить (т.е. *неблокирующих операций*), а для операций связанных с `IO` нужно использловать кастомный пул потоков, а лучше виртуальные потоки, либо же отдельные реактивные фреймворки.